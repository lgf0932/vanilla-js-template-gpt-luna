...
class UiThemeSwitch extends NovaElement {
  ...
  render() {
    ...
    const options = [
      { value: 'system', label: labels.system || '系统', icon: 'monitor' },
      { value: 'light', label: labels.light || '浅色', icon: 'sun' },
      { value: 'dark', label: labels.dark || '深色', icon: 'moon' },
    ];
    this.shadowRoot.innerHTML = `
      ...
      <div class="switch" role="radiogroup" aria-label="${escapeHtml(labels.label || '主题模式')}">${options.map((option) => `<button type="button" role="radio" aria-checked="${option.value === this.value}" class="${option.value === this.value ? 'active' : ''}" data-value="${option.value}"><ui-icon name="${option.icon}"></ui-icon><span>${escapeHtml(option.label)}</span></button>`).join('')}</div>
    `;
