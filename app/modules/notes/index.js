...
class NotesView extends NovaElement {
  ...
  async remove(note) {
    ...
    const confirmed = await dialog.open({ title: '删除笔记', message: `确定删除“${note.title}”吗？此操作无法撤销。`, confirmLabel: '删除', cancelLabel: '保留' });
    if (!confirmed) return;
    try {
