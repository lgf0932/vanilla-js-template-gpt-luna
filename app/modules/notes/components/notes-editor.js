...
class NotesEditor extends NovaElement {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <ui-card><span slot="title">${escapeHtml(isNew ? '新建笔记' : '编辑笔记')}</span><div class="editor"><ui-input name="title" label="标题" placeholder="给这条记录一个清晰标题" value="${escapeHtml(this.note.title)}" required></ui-input><ui-textarea name="content" label="内容" placeholder="记录上下文、链接或下一步…" rows="10" value="${escapeHtml(this.note.content)}"></ui-textarea><div class="actions"><ui-button class="cancel" variant="ghost">取消</ui-button><ui-button class="save"><ui-icon name="check"></ui-icon>保存</ui-button></div></div></ui-card>
    `;
    ...
    this.shadowRoot.querySelector('.save')?.addEventListener('click', () => {
      ...
      if (!title) {
        this.shadowRoot.querySelector('ui-input[name="title"]')?.setAttribute('error', '请输入标题');
        return;
      }
