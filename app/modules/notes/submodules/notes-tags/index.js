...
class NotesTagsView extends NovaElement {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <section><h1>${escapeHtml(t('notesTags.title', '笔记标签'))}</h1><p class="subtitle">${escapeHtml(t('notesTags.description', '用标签为不同主题建立轻量索引。'))}</p><ui-card><div class="empty"><ui-icon name="layers"></ui-icon><p>${escapeHtml(t('notesTags.empty', '还没有标签，创建第一组主题吧。'))}</p><ui-button class="create">${escapeHtml(t('notesTags.create', '新建标签'))}</ui-button></div></ui-card></section>
    `;
