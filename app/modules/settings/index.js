...
class SettingsView extends NovaElement {
  ...
  async update(input) {
    try {
      ...
      this.context?.shell?.showToast('设置已保存', 'success');
    } catch (error) {
  ...
  panelTemplate(t, state) {
    ...
    if (this.activeTab === 'security') return `<ui-card><div class="panel"><h2>${escapeHtml(t('settings.security.title', '安全会话'))}</h2><p>${escapeHtml(t('settings.security.description', '登录令牌只携带过期时间与随机数，不保存原始密码。'))}</p><ui-select class="session" label="${escapeHtml(t('settings.security.defaultDuration', '默认会话时长'))}"></ui-select><div class="actions"><ui-button class="security-save">保存偏好</ui-button></div></div></ui-card>`;
  ...
  bindPanel(state) {
    ...
    if (theme) { theme.value = state.display?.theme || 'system'; theme.addEventListener('theme-change', (event) => { applyTheme(event.detail.value); this.context.shell?.setTheme(event.detail.value); this.update({ display: { ...state.display, theme: event.detail.value } }); }); }
    const session = this.shadowRoot.querySelector('.session');
    if (session) { session.options = [{ value: '4h', label: '4 小时' }, { value: '8h', label: '8 小时' }, { value: '12h', label: '12 小时' }, { value: '24h', label: '24 小时' }, { value: '7d', label: '7 天' }, { value: '14d', label: '14 天' }, { value: '30d', label: '30 天' }, { value: '90d', label: '90 天' }, { value: 'session', label: '直到浏览器关闭' }]; session.value = state.sessionDefault || '24h'; }
