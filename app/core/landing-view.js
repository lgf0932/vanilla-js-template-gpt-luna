  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <main class="landing">
        ...
        <section class="hero"><div><div class="eyebrow"><ui-icon name="spark"></ui-icon>${escapeHtml(t('landing.eyebrow', '你的个人工作台'))}</div><h1>${escapeHtml(t('landing.title', '把想法，整理成下一步。'))}</h1><p class="lead">${escapeHtml(t('landing.lead', 'Nova 用清晰的模块、轻盈的交互和可靠的本地优先体验，让记录、复盘与行动回到同一个节奏里。'))}</p><div class="actions"><ui-button class="start">${escapeHtml(t('landing.start', '开始使用'))}<ui-icon name="arrow"></ui-icon></ui-button><ui-button class="explore" variant="ghost">${escapeHtml(t('landing.explore', '了解 Nova'))}</ui-button></div></div><div class="signal" aria-label="${escapeHtml(t('landing.preview.title', 'Nova 工作台预览'))}"><div class="signal-head"><span class="u-muted">${escapeHtml(t('landing.preview.title', '今日概览'))}</span><ui-badge variant="success">${escapeHtml(t('landing.preview.status', '运行正常'))}</ui-badge></div><div class="signal-grid"><div class="metric"><ui-icon name="note"></ui-icon><strong>12</strong><span>${escapeHtml(t('landing.preview.notes', '条灵感笔记'))}</span></div><div class="metric"><ui-icon name="layers"></ui-icon><strong>03</strong><span>${escapeHtml(t('landing.preview.modules', '个工作模块'))}</span></div><div class="metric"><ui-icon name="clock"></ui-icon><strong>24h</strong><span>${escapeHtml(t('landing.preview.window', '专注窗口'))}</span></div><div class="metric"><ui-icon name="check"></ui-icon><strong>100%</strong><span>${escapeHtml(t('landing.preview.control', '掌控感'))}</span></div></div></div></section>
